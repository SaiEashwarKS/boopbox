...
boopbox/
├── apps/
│   └── api/               Fastify 5 HTTP server (being migrated to Effect HTTP server)
├── packages/
...
Uses the **Fastify autoload pattern**: plugins from `src/plugins/` load first (registered via `fastify-plugin` / `fp` to share decorators), then routes from `src/routes/` (auto-prefixed by directory name).
...
|---|---|---|
| `packages/domain` | Shared types, schemas, pure search/filter logic | `effect` |
| `packages/api` | API contract definitions (`HttpApi`) | `@repo/domain`, `@effect/platform` |
| `apps/server` | Replaces `apps/api` — Effect HTTP server | `@repo/domain`, `@repo/api`, `@effect/platform-node` |
...
- **Catalog metadata** lives in a JSON file (`catalog.json`) loaded into memory at server startup. Each entry contains: name, category, tags, and filename.
- **Audio files** are served as static assets from the server (e.g., `public/sounds/`).
- If the catalog grows large enough that JSON-in-memory becomes awkward (thousands of sounds), step up to **SQLite** via `better-sqlite3` — still a single file, no external database.

### Curation

Sounds are manually curated from free/permissive sources (Freesound.org, Pixabay, MyInstants, etc.). There is no scraping pipeline or external API integration — sounds are downloaded, added to the repo or object storage, and registered in the catalog file.

## Key Decisions Log

- **Server migration:** Replacing Fastify with Effect HTTP server. No existing routes need migration (greenfield). Gains: typed clients, automatic OpenAPI generation.
